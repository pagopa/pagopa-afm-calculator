Feature: Payment Methods - Get List of Payment Methods
  ...
  Background: 
	Given the configuration "data.json"
  	Given the payment methods configuration "payment-methods-user-touchpoint-ok.json"

	Scenario: Simple search payment request using userTouchpoint IO, which is present in the payment methods configuration
		Given initial json
	  """
	  {
			"userTouchpoint": "IO",
